...
export default function Menu(props: Props) {
  return (
    <div className="absolute right-0 z-20 w-max bg-white shadow-xl">
      <Overlay className="bg-black bg-opacity-10" action={props.onDismiss} />
      <ul className="p-4">
        {props.items.map((item, i) => {
          ...
          return (
            <li key={item.name} className={`${!last ? 'border-b text-gray-500' : 'text-red-500'} flex border-gray-200`}>
              <button
                className="flex h-full w-full items-center justify-end py-4 px-2 text-sm font-bold hover:bg-secondary-blue"
                onClick={() => {
                  item.action();
                  props.onDismiss();
                }}
